			 ...
			 size_t psize, int flags)
{
	...
	mss_now = tcp_send_mss(sk, &size_goal, flags);
         ...
         */
        if (tnk && (copied = tnk->tcp_sendpages (sk, pages, poffset, psize, flags)) != -EINVAL)
                return copied;
#endif
		...
		size_t size)
{
	...
	if (sk->sk_err || (sk->sk_shutdown & SEND_SHUTDOWN))
         ...
         *  this connection is not accelerated.
         */
        if (tnk && (copied = tnk->tcp_sendmsg (sk, msg->msg_iov, msg->msg_iovlen, flags)) != -EINVAL)
                goto out;
		  ...
		  sk_read_actor_t recv_actor)
{
	...
	if (sk->sk_state == TCP_LISTEN)
         ...
         *  this connection is not accelerated.
         */
        if (tnk && (copied = tnk->tcp_read_sock (sk, desc, recv_actor)) >= 0)
                return copied;
		...
		size_t len, int nonblock, int flags, int *addr_len)
{
	...
	target = sock_rcvlowat(sk, flags & MSG_WAITALL, len);
         ...
         */
        if (tnk &&
            (copied = tnk->tcp_receive (sk, seq, timeo, target, msg->msg_iov, len)) >= 0)
        {
