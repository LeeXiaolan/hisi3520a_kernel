...
#endif
/*****************************************************************************/
static int hisfc300_wait_ready(struct hisfc300_host *host, struct hisfc_spi *spi)
{
	unsigned long regval;
	unsigned long deadline = jiffies + HISFC300_MAX_READY_WAIT_JIFFIES;
...
/*****************************************************************************/
static void hisfc300_enable_4byte_mode(struct hisfc300_host *host,
...
/*****************************************************************************/
static int hisfc300_write_enable(struct hisfc300_host *host,
...
static int hisfc300_quad_enable(struct hisfc300_host *host,
		struct hisfc_spi *spi)
{
	unsigned int regval;
	int count = 0;
	...
	do {
		hisfc300_write_enable(host, spi);

		hisfc300_write(host, HISFC300_CMD_INS, SPI_CMD_WRSR);
		hisfc300_write(host, HISFC300_CMD_DATABUF0, SPI_CMD_SR_QE);
		hisfc300_write(host, HISFC300_CMD_CONFIG,
		...
		count++;
	} while (!(regval & SPI_CMD_SR_QE) && (count < 300));
...
static void hisfc300_bus_config(struct hisfc300_host *host,
		struct hisfc_spi *spi, struct spi_operation *op,
		int clk_en, int writecmd, int readcmd, int isread)
{
	unsigned int regval = 0;
	...
	if (spi && op) {
		if (isread) {
			regval = HISFC300_BUS_CONFIG1_READ_IF_TYPE(op->iftype)
				| HISFC300_BUS_CONFIG1_READ_DUMMY_CNT(op->dummy)
				| HISFC300_BUS_CONFIG1_READ_INS(readcmd)
				| HISFC300_BUS_CONFIG1_READ_EN;
		} else {
			regval = HISFC300_BUS_CONFIG1_WRITE_DUMMY_CNT(op->dummy)
				| HISFC300_BUS_CONFIG1_WRITE_IF_TYPE(op->iftype)
				| HISFC300_BUS_CONFIG1_WRITE_INS(writecmd)
				| HISFC300_BUS_CONFIG1_WRITE_EN;
		}
	}
	...
	hisfc300_write(host, HISFC300_GLOBAL_CONFIG,
			HISFC300_GLOBAL_CONFIG_READ_DELAY(0)
			| (spi->addrcycle == 4
			? HISFC300_GLOBAL_CONFIG_ADDR_MODE_4B : 0));

	hisfc300_set_system_clock(host, op, clk_en);
}
/*****************************************************************************/
		...
		size_t *retlen, u_char *buf)
{
	...
	*retlen = 0;
	...
	while (len > 0) {
		...
		if (hisfc300_wait_ready(host, spi))
			goto fail;
		hisfc300_set_system_clock(host, spi->read, TRUE);
		...
		struct hisfc_spi *spi, unsigned long offset, int cmdindex)
{
	if (hisfc300_wait_ready(host, spi))
		return 1;
	hisfc300_write_enable(host, spi);
	hisfc300_set_system_clock(host, &spi->erase[cmdindex], TRUE);

	hisfc300_write(host, HISFC300_CMD_INS,
		...
		size_t *retlen, const u_char *buf)
{
	...
	*retlen = 0;
	...
	mutex_lock(&host->lock);
		...
		if (spi->addrcycle == 4)
				hisfc300_enable_4byte_mode(host, spi, 1);
	...
	if (to & HISFC300_REG_BUF_MASK) {
		...
		if (hisfc300_wait_ready(host, spi))
			goto fail;
		hisfc300_bus_config(host, spi, spi->write, 1, 0, 0, 0);
	...
	while (len > 0)	{
		...
		if (spi->addrcycle == 4)
				hisfc300_enable_4byte_mode(host, spi, 0);
		...
		size_t *retlen, u_char *buf)
{
	...
	*retlen = 0;
	...
	hisfc300_bus_config(host, spi, spi->read, 1, 0, spi->read->cmd, 1);
	...
	while (len > 0) {
		while (from >= spi->chipsize) {
			...
			if (hisfc300_wait_ready(host, spi))
				goto fail;
			hisfc300_bus_config(host, spi, spi->read,
					1, 0, spi->read->cmd, 1);
...
static int hisfc300_probe(struct hisfc300_host *host)
{
	...
	hisfc300_set_system_clock(host, NULL, TRUE);
