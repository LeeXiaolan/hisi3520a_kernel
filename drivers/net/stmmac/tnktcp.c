...
static void tnk_tcp_connection_idle_timer_handler(unsigned long index)
{
	struct sock *sk = tnk_ct_get_sock(index);
	struct tnkinfo *t = &sk->sk_tnkinfo;
	...
	if (t->state == TNKINFO_STATE_ACTIVE) {
		/* We've been idle for a minute, but check again
		 * in case something has just popped into the queues
		 */
		if (!skb_queue_empty(&t->entry->receive_queue)) {
			TNK_DBG("%s queues non-empty, re-activating timer\n",
				__func__);
			mod_timer(&t->idle_timer,
				  jiffies + tnk_tcp_connection_idle_time);
		} else {
			CONTROL_DBG
			    ("%s idle time expired, handing connection %d back"
			     " to Linux stack\n",
			     __func__, t->entry->index);

			/*  This handler runs in softirq context, so we can't
			 *  afford a graceful shutdown */
			if (tnk_tcp_close(sk, 0)) {
				/* If we didn't get a clean close-down, we
				 * still have data in the Tx pipeline, so raise
				 * an error
				 *
				 * NOTE - this should also ensure that
				 * connections which are blocked by
				 * ifconfig eth* down are eventually freed up
				 */
...
int tnk_tcp_close(struct sock *sk, int graceful)
{
	...
	if (t->state == TNKINFO_STATE_STOPPED) {
		...
		TNK_DBG("%s deleting idle timer\n", __func__);
		del_timer(&t->idle_timer);
...
int tnk_tcp_update(struct sock *sk, int len)
{
	...
	TNK_DBG("%s setup idle timer\n", __func__);
	setup_timer(&t->idle_timer,
		    tnk_tcp_connection_idle_timer_handler, t->entry->index);
	mod_timer(&t->idle_timer, jiffies + tnk_tcp_connection_idle_time);
