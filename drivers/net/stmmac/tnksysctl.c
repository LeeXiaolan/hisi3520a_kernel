...
int tnk_tcp_connection_idle_time = TNK_TCP_CONNECTION_IDLE_TIME;
int tnk_tcp_persist_time = TNK_TCP_PERSIST_TIME;
int tnk_threshold;
module_param(tnk_threshold, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(tnk_threshold, "min:100, typical:10x1024");
...
#endif
int tnk_tx_timeout_limit = TNK_TX_TIMEOUT_LIMIT;
...
static struct ctl_table tnk_table[] = {
	{
		.procname	= "tnk_tcp_connection_idle_time",
		.data		= &tnk_tcp_connection_idle_time,
		.maxlen		= sizeof(int),
		.mode		= 0644,
...
#endif
	{
		.procname	= "tnk_tx_timeout_limit",
		.data		= &tnk_tx_timeout_limit,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= proc_dointvec
	},
