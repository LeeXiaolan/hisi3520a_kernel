...
#define get_bus_clk()({\
	...
	if ((tmp_reg & 0xc) == 0 || (tmp_reg & 0xc) == 0x4) {\
		busclk = foutvco/6;\
...
static void *ctr;
static void godarm_update_sched_clock(void)
{
	u32 cyc = readl(ctr);
...
static void __init godarm_sched_clock_init(void* reg, unsigned long rate)
{
	ctr = reg;
...
static int __init godarm_clocksource_init(void)
{
	...
	godarm_clocksource.mult =
		clocksource_khz2mult(timer1_clk_khz, godarm_clocksource.shift);

	godarm_sched_clock_init((void *)CFG_TIMER_VABASE + REG_TIMER1_VALUE,
			timer1_clk_khz * 1000);

	clocksource_register(&godarm_clocksource);
...
void __init godarm_init(void)
{
	...
	/* Due to bootrom, godarm use fixed uart clk */
