...
static void *ctr;
static void godnet_update_sched_clock(void)
{
	u32 cyc = readl(ctr);
...
static void __init godnet_sched_clock_init(void* reg, unsigned long rate)
{
	ctr = reg;
...
static int __init godnet_clocksource_init(void)
{
	...
	godnet_clocksource.mult =
		clocksource_khz2mult(timer1_clk_khz, godnet_clocksource.shift);

	godnet_sched_clock_init((void *)CFG_TIMER_VABASE + REG_TIMER1_VALUE,
			timer1_clk_khz * 1000);

	clocksource_register(&godnet_clocksource);
...
void __init godnet_init(void)
{
	...
	/* Due to bootrom, godnet use fixed uart clk */
	uart_clk_init(6000000);
