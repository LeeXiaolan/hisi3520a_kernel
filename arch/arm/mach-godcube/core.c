...
static unsigned long long godcube_cycles_2_ns(unsigned long long cyc)
{
...
unsigned long long sched_clock(void)
{
	...
	return godcube_cycles_2_ns((ticks64 << 32) | ticks2);
}

static DEFINE_CLOCK_DATA(cd);
static void *ctr;
static void godcube_update_sched_clock(void)
{
	u32 cyc = readl(ctr);
...
static void __init godcube_sched_clock_init(void* reg, unsigned long rate)
{
	ctr = reg;
	init_fixed_sched_clock(&cd, godcube_update_sched_clock,
			32, rate, godcube_clocksource.mult,
			godcube_clocksource.shift);
}
...
static int __init godcube_clocksource_init(void)
{
	...
	godcube_clocksource.mult =
		clocksource_khz2mult(timer1_clk_khz, godcube_clocksource.shift);

	godcube_sched_clock_init((void *)CFG_TIMER01_VABASE
			+ REG_TIMER1_VALUE, timer1_clk_khz * 1000);

	clocksource_register(&godcube_clocksource);
